...
if __name__ != "__main__":
    raise ImportError("This is a script and cannot be imported.")
...
# Load up the configuration, this includes parsing any command line arguments.
...
# Save the session if we had to login or if we replenished our cache (because
# they are tied together artificially by our design).
if save_session:
    session.save()

# Enter the shell or execute a command.
if config.CONFIG.get("shell"):
    import apiclient.shell
    new_shell = apiclient.shell.APIShell(session)
